...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 5: conflict-targeted idle-gap.  For each aircraft a,
        # find the LATEST front-aircraft finish that could block it, and
        # try Δ_a so a starts exactly after that front clears.  This is
        # the "land slot-2 after slot-1" move targeted at the dominant
        # blocker rather than guessed by a fixed grid.
        sched_by_aid = {x["id"]: x for x in best_sol["aircraft"]}
        ...
        for aid in aircraft_ids:
            if not _time_left():
                break
            p_a = assignment[aid]
            front_positions = arc_to.get(p_a, [])
            if not front_positions:
                continue
            base = aircraft[aid]["earliest_start"]
            front_finishes: list[float] = []
            for f_aid, f_sched in sched_by_aid.items():
                if f_aid == aid:
                    continue
                if f_sched["position"] in front_positions:
                    front_finishes.append(f_sched["finish"])
            if not front_finishes:
                continue
            # Try each front finish + eta as the target start of aid.
            front_finishes.sort()
            tried = set()
            for fin in front_finishes:
                d_target = fin + eta - base
                if d_target <= 0:
                    continue
                d_key = round(d_target, 2)
                if d_key in tried:
                    continue
                tried.add(d_key)
                new_overrides = dict(start_overrides)
                new_overrides[aid] = base + d_target
                sol = _rebuild_job(assignment, instance, params, order=order,
                                   start_overrides=new_overrides)
                obj = _objective_job(sol, params)
                if _accept(sol, obj):
