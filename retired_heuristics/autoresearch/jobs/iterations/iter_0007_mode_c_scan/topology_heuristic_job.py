...
def _solve_single(
    ...
    # Mode-C scan: try greedy insertion of Mode-C events on the incumbent.
    best_sol, best_obj = _mode_c_scan(
        best_assignment, None, instance, params, best_sol, best_obj,
    )

    # ---- LNS perturbation loop ----
    # Destroy K random aircraft from the incumbent, rebuild via the same
    # GRASP candidate scoring, run LS, accept on strict improvement.
    ...
    while _remaining() > 0.0:
        ...
        if _remaining() > 0.0:
            new_sol, new_obj = _mode_c_scan(
                new_assignment, None, instance, params, new_sol, new_obj,
            )
...
def _mode_c_scan(
    ...
    """Post-LS Mode-C scan: try inserting Mode-C events one at a time.

    For every (rear R, front F) pair such that F's position blocks R's
    position and R currently has delay > 0, look for an interruptible
    job j of F whose ``[j.start, j.finish]`` spans R's natural arrival
    time (current start minus current delay).  Try the rebuild with one
    Mode-C event ``(F, j, R)`` added — that extends j by δ, exempts the
    (R, F) pair from Mode-A delay, and charges 2 movements.  Accept the
    move iff it strictly improves the objective AND passes the checker.

    Greedy: scan until no single Mode-C addition helps."""
    if _check_solution is None:
        return best_sol, best_obj

    aircraft     = instance["aircraft"]
    arc_set      = set(instance["blocking_arcs"])
    raw_instance = instance["_raw"]
    extensions:  dict[str, dict[str, int]]  = {}
    mode_c_skip: set[tuple[str, str]]       = set()

    while True:
        improved = False
        by_aid = {a["id"]: a for a in best_sol["aircraft"]}
        aids   = list(by_aid.keys())

        for r_aid in aids:
            r_pos = by_aid[r_aid]["position"]
            for f_aid in aids:
                if f_aid == r_aid:
                    continue
                f_pos = by_aid[f_aid]["position"]
                if (f_pos, r_pos) not in arc_set:
                    continue
                if (r_aid, f_aid) in mode_c_skip:
                    continue
                interruptible = [j for j in aircraft[f_aid]["chain"]
                                 if j["interruptible"]]
                if not interruptible:
                    continue

                for j in interruptible:
                    new_ext = {fa: dict(d) for fa, d in extensions.items()}
                    new_ext.setdefault(f_aid, {})
                    new_ext[f_aid][j["id"]] = new_ext[f_aid].get(j["id"], 0) + 1
                    new_skip = mode_c_skip | {(r_aid, f_aid)}

                    sol = _rebuild_job(
                        assignment, instance, params, order=order,
                        extensions=new_ext, mode_c_skip=new_skip,
                    )
                    ...
                    probe = dict(sol)
                    probe["objective"] = obj
                    if not _check_solution(probe, raw_instance)["compliant"]:
                        continue

                    extensions  = new_ext
                    mode_c_skip = new_skip
                    best_sol    = sol
                    best_obj    = obj
                    improved    = True
                    break
...
def _local_search(
    ...
    while improved and _time_left():
        ...
        for k in range(len(order) - 1):
            ...
            sol = _rebuild_job(assignment, instance, params, order=trial_order)
            obj = _objective_job(sol, params)
            if _accept(sol, obj):
                order    = trial_order
                best_obj = obj
...
def _rebuild_job(
    ...
    extensions:  dict[str, dict[str, int]] | None = None,
    mode_c_skip: set[tuple[str, str]] | None  = None,
) -> dict:
    ...
    ----------
    extensions
        ``{front_aid: {job_id: kappa}}`` — number of Mode-C extensions
        applied to each interruptible job.  Each kappa extends the job's
        effective duration by ``kappa * delta``.  Total movements charged
        = ``2 * sum(kappa)``.
    mode_c_skip
        ``{(rear_aid, front_aid), ...}`` — rear-front pairs whose conflict
        has been absorbed by a Mode-C event and therefore should NOT be
        resolved via Mode-A delay in the rear- or front-side passes.
    ...
    mode_c_skip = mode_c_skip if mode_c_skip is not None else set()
    ...
    # Total Mode-C events × 2 (each event = one in + one out movement).
    total_movements = 2 * sum(sum(d.values()) for d in extensions.values())
    ...
    for aid in placed_aids:
        ...
        # Per-aircraft extension table; default 0 for every job.
        ext_for_aid = extensions.get(aid, {})

        # Build provisional job schedule with Mode-C extensions applied.
        chain = ac["chain"]
        prov_jobs = []
        t = t_start
        for j in chain:
            kappa = ext_for_aid.get(j["id"], 0)
            dur_eff = j["duration"] + kappa * delta
            prov_jobs.append({"id": j["id"], "start": t, "finish": t + dur_eff,
        ...
        for _outer in range(outer_max_iter):
            ...
            if extra_delay > 0:
                ...
                for j_idx, j in enumerate(chain):
                    ...
                    prov_jobs[j_idx]["finish"] = t + dur_eff
                    t += dur_eff
...
def _resolve_rear_interactions(
    ...
    delay_extra = 0.0
    eta         = params["eta"]
    mode_c_skip = mode_c_skip or set()
        ...
        for f_pos in front_positions:
            for front_aid, sch in scheduled.items():
                if sch["position"] != f_pos:
                    continue
                if (aid, front_aid) in mode_c_skip:
                    continue   # Mode-C absorbs this conflict; no Mode-A delay
                f_start  = sch["start"]
                f_finish = sch["finish"]
        ...
        for r_pos in rear_positions:
            for rear_aid, sch in scheduled.items():
                ...
                if rear_aid in rears_done:
                    continue
                if (rear_aid, aid) in mode_c_skip:
                    continue   # Mode-C absorbs this conflict; no Mode-A push
                has_conflict = False
