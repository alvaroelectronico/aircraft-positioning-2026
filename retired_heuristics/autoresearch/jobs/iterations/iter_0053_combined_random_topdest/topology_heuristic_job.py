...
def _solve_single(
    ...
    while _remaining() > 0.0:
        ...
        # Seven-mode cycling — combine random+topdest as new mode.
        ...
        #   6:    random K + topdest combined destroy + uniform repair
        mode = kick_idx % 7
        kick_idx += 1
        if mode == 0 or mode == 3:
            ...
            counts2: dict[str, list[str]] = {}
            for aid, p in best_assignment.items():
                counts2.setdefault(p, []).append(aid)
            ranked2 = sorted(counts2.items(), key=lambda kv: -len(kv[1]))
            topdest_set = set(aid for _, aids in ranked2[:1] for aid in aids)
            random_set  = set(rng.sample(list(best_assignment.keys()), k))
            destroyed = list(topdest_set | random_set)
            repair = "uniform"
