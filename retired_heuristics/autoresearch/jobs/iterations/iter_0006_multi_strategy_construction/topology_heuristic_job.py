...
# Construction-order strategies, cycled across multi-starts.  Each one
# produces a different greedy trajectory; combined with the RCL pick they
# give n_starts genuinely diverse starting points instead of n_starts
# slightly-randomised "heaviest-first" runs.
_CONSTR_STRATEGIES = (
    "heaviest",   # total_duration DESC  (original behaviour)
    "lightest",   # total_duration ASC
    "edd",        # target_finish ASC
    "earliest",   # earliest_start ASC
    "slack",      # (target_finish - earliest_start - total_duration) ASC
...
class TopologyHeuristicJob:
    ...
    def solve(self, instance_data: dict) -> dict:
        ...
        for k in range(n_starts):
            ...
            strategy  = _CONSTR_STRATEGIES[k % len(_CONSTR_STRATEGIES)]
            sol, obj  = _solve_single(instance, params, per_start, rng,
                                      log_lines, strategy)
...
def _solve_single(
    ...
    """One GRASP iteration: construct (using the given ordering strategy),
    run the LS portfolio, then a sequence of LNS-style destroy-and-rebuild
    perturbations that re-use the remaining budget.  The strategy is
    preserved across LNS rebuilds so each multi-start replicate keeps its
    bias."""
    ...
    while _remaining() > 0.0:
        ...
        new_assignment = _construct(
            ...
            partial_assignment=partial, strategy=strategy,
        )
        if _remaining() <= 0.0:
            break
...
def _construct(
    ...
    are kept fixed; the remaining ones are placed greedily in an order
    determined by ``strategy``, using the standard candidate-scoring +
    biased-random RCL pick."""
    aircraft   = instance["aircraft"]
    positions  = instance["positions"]
    ...
    to_place = [aid for aid in aircraft.keys() if aid not in fixed]
    if strategy == "heaviest":
        to_place.sort(key=lambda aid: -aircraft[aid]["total_duration"])
    elif strategy == "lightest":
        to_place.sort(key=lambda aid:  aircraft[aid]["total_duration"])
    elif strategy == "edd":
        to_place.sort(key=lambda aid:  aircraft[aid]["target_finish"])
    elif strategy == "earliest":
        to_place.sort(key=lambda aid:  aircraft[aid]["earliest_start"])
    elif strategy == "slack":
        to_place.sort(key=lambda aid: (aircraft[aid]["target_finish"]
                                       - aircraft[aid]["earliest_start"]
                                       - aircraft[aid]["total_duration"]))
...
def _local_search(
    ...
    # If the construction landed on an assignment whose Mode-A rebuild is
    # itself infeasible under the checker, mark best_obj as +inf so any
    # compliant LS move strictly dominates it.  Otherwise non-compliant
    # initials can survive the search and contaminate the multi-start best.
    if _check_solution is not None:
        probe = dict(best_sol)
        probe["objective"] = best_obj
