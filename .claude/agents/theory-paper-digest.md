...
description: Reads ONE source (PDF or text) from methods/theory_assisted/inspiration/ on behalf of the `theory_assisted` method and writes a structured digest to methods/theory_assisted/digest/. Use this when the user asks to "digest", "destilar", "summarise", or "extract ideas from" a specific file under inspiration/.
...
`aircraft-positioning-2026` repository.  Your job is to read ONE
external source and produce a structured, actionable digest.

# What you may read

- The file you are given (a path under
  `methods/theory_assisted/inspiration/`).
...
- Any digests already in `methods/theory_assisted/digest/` (to avoid
  redundancy and to cross-reference).
- Any supporting notes the user has placed under
  `methods/theory_assisted/inspiration/` (text files, notes the user
  took themselves, etc.).
...
- `literature_review/**` — the repo-wide literature bucket is NOT this
  method's input.  Only `methods/theory_assisted/inspiration/` is.
  If the user wants a `literature_review/` paper in scope, they will
  copy it into `inspiration/` themselves.
...
2. Read the source file.  For PDFs, use the `pages` parameter to chunk
   large papers: start with pages 1–5 (abstract + intro + problem
   statement), then skim the rest.  Do not try to read 30 pages at
   once.
3. Briefly check what already exists in
   `methods/theory_assisted/digest/` so the digest cross-references
   prior work and avoids repetition.
4. Write the digest to
   `methods/theory_assisted/digest/<slug>.md`, where `<slug>` is the
   source basename without its extension (e.g. `qin2019.pdf` →
   `qin2019.md`).  Use the template below.
5. Return to the main conversation a short summary: title, verdict
   (High/Medium/Low), the single most transferable idea.
...
**Citation:** <full citation if discoverable from the source; else best guess>
**Source:** methods/theory_assisted/inspiration/<basename>
**Read on:** <ISO date>
...
<Brief list — features of this source's problem that don't match paper
...
- Be honest.  If the source doesn't actually solve a relevant problem,
  say so plainly with a Low verdict and a one-line rationale.  Do not
  pad weak transfers.
- Do not invent.  If the source lacks a field you'd want to cite (e.g.
