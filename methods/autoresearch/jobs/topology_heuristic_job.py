...
def _solve_single(
    ...
    n_ac = len(aircraft)
    kick_sizes = [1, 2, max(1, n_ac // 3), max(1, n_ac // 2)]
    kick_idx = 0
    while _remaining() > 0.0:
        k = kick_sizes[kick_idx % len(kick_sizes)]
        # Five-mode cycling (dropped topdest under no-restart LS):
        #   0, 2: random destroy + greedy repair
        #   1, 3: random destroy + uniform repair
        #   4:    FULL restart (destroy all, uniform repair)
        mode = kick_idx % 5
        kick_idx += 1
        if mode == 0 or mode == 2:
...
def _local_search(
    ...
    while improved and _time_left():
        ...
        for aid in aircraft_ids:
            ...
            if improved:
                break
        # iter_0067: do NOT restart from op 1 on improvement — continue to
        # op 2.  Saves time on chain/hub where ops 1, 2 are cheap and the
        # restart-on-improvement burns budget redoing op 1 trials.
        ...
        for i in range(len(aircraft_ids)):
            ...
            if improved:
                break
        # iter_0067: no restart-after-improvement; fall through to op 3.
        ...
        for k in range(len(order) - 1):
            ...
            if _accept(sol, obj):
                ...
                improved = True
                break
        # iter_0067: no restart; fall through to op 4.
        ...
        # aircraft and reinsert at every other slot.
        ...
        # Operator 6: EDD/slack repair per position.  Replace each
        # same-position block with one of three full reorderings:
        # EDD (target_finish ASC), slack ASC, delay-ratio DESC.
        delay_of = {a["id"]: a.get("delay", 0.0) for a in best_sol["aircraft"]}
        dur_of   = {aid: max(1.0, aircraft[aid]["total_duration"]) for aid in aircraft}
        blocks2: dict[str, list[str]] = {}
        for aid in order:
            blocks2.setdefault(assignment.get(aid, "?"), []).append(aid)
        for pos, pos_aids in blocks2.items():
            if not _time_left() or len(pos_aids) < 2:
                continue
            pos_set = set(pos_aids)
            candidates = [
                sorted(pos_aids, key=lambda aid: aircraft[aid]["target_finish"]),
                sorted(pos_aids, key=lambda aid:
                       aircraft[aid]["target_finish"] - aircraft[aid]["earliest_start"]),
                sorted(pos_aids, key=lambda aid:
...
def _resolve_rear_interactions(
    ...
    total_duration = f_r_initial - s_r_first
    eta            = params["eta"]

    # Per-access Mode-A semantics matching the checker (RQ07_v2).  Each
    # ACCESS instant τ of the rear must satisfy
    #     τ ≤ f_start - eta   OR   τ ≥ f_finish + eta
    # for every overlapping front aircraft.  Two options per front:
    #   z+ : tau_in ≥ f_finish + eta   (rear lands after front clears)
    #   z- : tau_in ≤ f_start - eta AND tau_out ≥ f_finish + eta
    #        (engulfing rear: stay overlaps front but accesses are on
    #         opposite sides — the MILP staggering pattern on chain).
    #
    # Compute the MINIMUM tau_in ≥ s_r_first that satisfies every front
    # via either option.  Delay = max(0, optimal_tau_in - s_r_first).
    fronts = []
    for f_pos in front_positions:
        for front_aid, sch in scheduled.items():
            if sch["position"] == f_pos:
                fronts.append((sch["start"], sch["finish"]))
    if not fronts:
        return {}, 0.0, 0

    def _feasible(tau_in: float) -> bool:
        tau_out = tau_in + total_duration
        for f_s, f_f in fronts:
            in_ok  = (tau_in  <= f_s - eta) or (tau_in  >= f_f + eta)
            out_ok = (tau_out <= f_s - eta) or (tau_out >= f_f + eta)
            if not (in_ok and out_ok):
                return False
        return True

    # Build candidate tau_in values from front-boundary arithmetic.
    candidates = {s_r_first}
    for f_s, f_f in fronts:
        candidates.add(f_f + eta)                    # z+ floor for this front
        z_minus_lo = f_f + eta - total_duration
        if z_minus_lo > s_r_first:
            candidates.add(z_minus_lo)               # z- floor for this front

    chosen = None
    for cand in sorted(c for c in candidates if c >= s_r_first):
        if _feasible(cand):
            chosen = cand
            break

    if chosen is None:
        # Defensive fallback: iterative push-past-front (old behaviour).
        # Should rarely fire because the candidate set covers all the
        # boundaries the per-access constraints can hit.
        delay_extra = 0.0
        max_iter = 4 * max(1, len(scheduled))
        for _ in range(max_iter):
            needed = 0.0
            tau_in_now = s_r_first + delay_extra
            for f_s, f_f in fronts:
                if tau_in_now + total_duration + eta <= f_s:
                    continue
                if tau_in_now >= f_f + eta:
                    continue
                want = f_f + eta - tau_in_now
                if want > needed:
                    needed = want
            if needed <= 0:
                break
    ...
    return {}, max(0.0, chosen - s_r_first), 0
    ...
    for _ in range(max_iter):
        ...
        for r_pos in rear_positions:
            for rear_aid, sch in scheduled.items():
                ...
                # determines the needed shift.
                F_s = prov_jobs[0]["start"]
                F_f = prov_jobs[-1]["finish"]
                worst_tau = None
                for tau in (sch["start"], sch["finish"]):
                    if tau <= F_s - eta or tau >= F_f + eta:
                        continue
                    if worst_tau is None or tau > worst_tau:
                        worst_tau = tau
                if worst_tau is not None:
                    needed = worst_tau + eta - F_s
