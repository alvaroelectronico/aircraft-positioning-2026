...
def _solve_single(
    ...
    while _remaining() > 0.0:
        ...
        # Six modes cycling: random+greedy, random+uniform, topdest+uniform.
        # topdest = "destroy every aircraft at the two most-loaded positions"
        # — forces rebalancing when the incumbent has stacked positions.
        mode = kick_idx % 3
        kick_idx += 1
        if mode == 0:
