...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 5: global order swap.  For any pair of aircraft at
        # DIFFERENT positions, swap their slots in the schedule order.
        # The rebuild processes aircraft in this order, so when a (front,
        # rear) pair appears in different orders the resolver takes
        # different paths through Mode-A — sometimes finding tighter
        # schedules.
        for i in range(len(order)):
            ...
            for j in range(i + 1, len(order)):
                ...
                if assignment.get(a1) == assignment.get(a2):
                    continue  # same-position handled by Op 3
                trial_order = list(order)
