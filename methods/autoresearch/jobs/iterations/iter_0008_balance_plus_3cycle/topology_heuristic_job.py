...
class TopologyHeuristicJob:
    ...
    def solve(self, instance_data: dict) -> dict:
        ...
        for k in range(n_starts):
            ...
            # Alternate construction scoring: half "topology" (penalise heavy
            # aircraft at high-blocking positions, original behaviour) and
            # half "balance" (penalise stacking at positions that already
            # carry many aircraft).  "balance" is decisive on chain-style
            # topologies where the MILP optimum is even distribution.
            scoring = "topology" if (k % 2 == 0) else "balance"
...
def _solve_single(
    ...
    rng:       random.Random,
    log_lines: list[str],
    scoring:   str = "topology",
) -> tuple[dict, float]:
    """One GRASP iteration: construct (with the given scoring strategy),
    run the LS portfolio, then a sequence of LNS-style destroy-and-rebuild
    perturbations that re-use the remaining budget."""
    ...
    while _remaining() > 0.0:
        ...
        new_assignment = _construct(
            ...
            partial_assignment=partial, scoring=scoring,
        )
        if _remaining() <= 0.0:
            break
...
def _construct(
    ...
    ``scoring`` chooses the per-candidate penalty layered on top of the
    trial rebuild's objective:
      * "topology" — weight × ac.duration × blocking_load[p]  (original)
      * "balance"  — weight × ac.duration × count_at_p²       (discourages
                     stacking; favours even per-position counts, which is
                     what the MILP optimum looks like on chain topologies)
    """
    aircraft   = instance["aircraft"]
    positions  = instance["positions"]
    ...
    w_extra = params["weight_topology"]
    for aid in to_place:
        ac = aircraft[aid]
        # Per-position count of aircraft already assigned (for balance scoring).
        ...
        for p in positions:
            ...
            if scoring == "balance":
                extra_pen = (w_extra * ac["total_duration"]
                             * (count_at[p] + 1) * (count_at[p] + 1))
            else:
                extra_pen = (w_extra * ac["total_duration"]
                             * blocking_load.get(p, 0))
            candidates.append((p, base_cost + extra_pen))
...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 4: 3-cycle position rotation.
        # For each ordered triple of aircraft with distinct positions, try the
        # rotation (a→pos(b), b→pos(c), c→pos(a)) and its reverse.  Reaches
        # 3-aircraft neighbourhoods that single-move and 2-opt swap cannot
        # explore (e.g. balance-shifting moves on chain topologies).
        n = len(aircraft_ids)
        for i in range(n):
            if not _time_left():
                break
            ...
            for j in range(n):
                ...
                for kk in range(n):
                    ...
                    if pk == pi or pk == pj:
                        continue
                    # Try forward rotation a→pj, b→pk, c→pi.
                    trial = dict(assignment)
                    trial[ai] = pj
                    trial[aj] = pk
                    trial[ak] = pi
