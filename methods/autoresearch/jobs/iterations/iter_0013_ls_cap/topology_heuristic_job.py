...
def _solve_single(
    ...
    while _remaining() > 0.0:
        ...
        if _remaining() <= 0.0:
            break
        # Cap each LNS-internal LS at 2 s so the LNS gets many shots per
        # 20 s instance budget instead of one or two deep ones.  Initial
        # LS (above) still uses the full remaining budget so the incumbent
        # is well-polished before perturbations begin.
        new_sol, new_obj = _local_search(
            new_assignment, None, instance, params, min(_remaining(), 2.0),
        )
