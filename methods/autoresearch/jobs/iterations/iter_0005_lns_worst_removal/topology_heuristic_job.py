...
def _solve_single(
    ...
    # strict improvement.  Destroy operators alternate between:
    #   * random        — uniform sample (diversification).
    #   * worst-removal — top-K by delay + their blocking-arc neighbours
    #                     (intensification on the cluster that hurts most).
    ...
    while _remaining() > 0.0:
        ...
        if kick_idx % 2 == 0:
            destroyed = _destroy_random(best_assignment, k, rng)
        else:
            destroyed = _destroy_worst(
                best_sol, best_assignment, k, instance, rng,
            )
        kick_idx += 1
...
def _destroy_random(
    ...
    """Uniformly sample ``k`` aircraft ids from the incumbent assignment."""
    aids = list(assignment.keys())
    k = min(k, len(aids))
    return set(rng.sample(aids, k))
...
def _destroy_worst(
    ...
    """Pick the k aircraft with the largest delay in ``sol``; expand the
    set with each picked aircraft's blocking-arc neighbours sharing a
    front- or rear-arc position (capped at 2k total)."""
    ranked = sorted(sol["aircraft"], key=lambda a: -a.get("delay", 0.0))
    picks  = [a["id"] for a in ranked[:k]]
    chosen: set[str] = set(picks)

    arc_from = instance["arc_from"]
    arc_to   = instance["arc_to"]
    pos_to_aids: dict[str, list[str]] = {}
    for aid, p in assignment.items():
        pos_to_aids.setdefault(p, []).append(aid)
    ...
    for aid in picks:
        ...
        if p is None:
            continue
        # Neighbouring positions across blocking arcs (both directions).
        nbr_positions = list(arc_from.get(p, [])) + list(arc_to.get(p, []))
        for q in nbr_positions:
            for other in pos_to_aids.get(q, []):
    ...
    # Always destroy something even if delays are all 0 (degenerate).
    if not chosen:
        return _destroy_random(assignment, max(1, k), rng)
    return chosen
