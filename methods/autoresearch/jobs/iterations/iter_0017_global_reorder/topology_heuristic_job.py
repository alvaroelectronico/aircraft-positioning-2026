...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 5: global reorder.  Try replacing the scheduling sequence
        # with one of four candidate orderings:
        #   - target_finish ASC   (EDD)
        #   - total_duration DESC (heaviest first)
        #   - blocking_load[assignment[aid]] ASC (rear-most positions first)
        #   - blocking_load[assignment[aid]] DESC (front-most positions first)
        # The two blocking-load variants are the key ones for chain/hub:
        # scheduling one direction lets the front- or rear-side resolver
        # see the other side already in place, which the local intra-pos
        # swaps can't simulate.
        candidate_orders = [
            sorted(aircraft_ids, key=lambda aid: aircraft[aid]["target_finish"]),
            sorted(aircraft_ids, key=lambda aid: -aircraft[aid]["total_duration"]),
            sorted(aircraft_ids, key=lambda aid: blocking_load.get(assignment[aid], 0)),
            sorted(aircraft_ids, key=lambda aid: -blocking_load.get(assignment[aid], 0)),
        ]
        for cand_order in candidate_orders:
            ...
            if cand_order == order:
                continue
            sol = _rebuild_job(assignment, instance, params, order=cand_order,
                               start_overrides=start_overrides)
            obj = _objective_job(sol, params)
            if _accept(sol, obj):
                order    = cand_order
