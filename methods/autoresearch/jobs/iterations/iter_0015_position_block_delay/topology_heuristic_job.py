...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 5: position-block delay.  Try shifting every aircraft at
        # a position by the SAME Δ — replicates the MILP's pattern of
        # "slot 2 starts after slot 1 fully clears" by delaying the
        # second wave of aircraft at a given position together.
        block_deltas = (5.0, 20.0, 50.0, 100.0)
        for pos in positions:
            if not _time_left():
                break
            pos_aids = [aid for aid in aircraft_ids if assignment.get(aid) == pos]
            if len(pos_aids) < 2:
                continue
            # Apply Δ only to the LATER aircraft in the position block —
            # the first one already starts at pos_free=0 so a delta would
            # just be wasted tardiness.
            cur_order = order or aircraft_ids
            ordered_block = [aid for aid in cur_order if aid in pos_aids]
            later = ordered_block[1:]
            for d in block_deltas:
                new_overrides = dict(start_overrides)
                for aid in later:
                    new_overrides[aid] = aircraft[aid]["earliest_start"] + d
