...
Full **10-seed** battery (v05 = light-objective + incremental zero decode,
strict-60 s deadline), paired against the cached job-level MILP.  Generated by
`experiments/ta_battery_log.py`, which emits the single consolidated `.log`
(v01 structure: CONFIGURATIONS → gap tables → SUMMARY with the MILP row
interleaved before each heuristic row, seed-first).  The shared `gap_summary.py`
is hard-wired to v01's `igvnd_*` labels, so the wrapper swaps in `ta_igvnd_*`.

The v04/v05 throughput work (+14–49 % more search iterations) leaves these
numbers **statistically unchanged** vs the earlier v03.1 battery (deltas ±0.1–
1.7 pt, both directions): the search is **quality-saturated at 60 s** — more
iterations no longer find better solutions, they only shift the time-limited
stochastic trajectory.  Throughput is therefore no longer the quality
bottleneck; the remaining lever is architectural (decoder expressiveness).
...
| Methods compared  | `ta_igvnd_*` (v05) vs cached `milp_job_*` |
| Weight profiles   | wMK (100/1/1) · wDLY (1/100/1) · wMOV (1/1/100) |
| Budget            | 60 s per run (strictly enforced — max observed 60.0 s) |
| Metric            | mean relative gap `(MILP − heur)/MILP` over 10 seeds (>0 ⇒ heuristic better) |
| Instances         | `data/instances_202605_02` |
| Log               | `outputs/logs/instances_main_methods_20260616_210727_iterated_greedy_vnd_v02_02.log` |
...
| **triangle_tight_R20**     | **+16.6 %** | **+9.5 %**  | **+12.6 %** |
| **triangle_tight_R30**     | **+36.1 %** | **+35.1 %** | **+34.3 %** |
...
- **Large instances (R20/R30) — dominant** across all three profiles
  (full_R20 +34–55 %, triangle_R20 +10–17 %, triangle_R30 +34–36 %): the 60 s
