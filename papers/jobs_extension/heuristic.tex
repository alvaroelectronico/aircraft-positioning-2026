...
\While{time remains}{
  ...
  $(\pi, \sigma) \leftarrow \textsc{Search}(\pi, \sigma, \textsc{Decode}_0)$;
     ...
     \tcp*{VND descent + IG destroy/rebuild (Alg.~\ref{alg:search}); the zero-movement floor}
...
minimises the decoded objective.  The perturbed state is re-optimised by the VND and accepted as
...
a time slice of $T/8$ ($R \le 10$), $T/4$ ($R \le 20$) or $T/3$ (larger) and
ends early on its own stale counter, so small instances complete many
independent restarts while large ones receive only a few --- with no
