...
\begin{enumerate}[leftmargin=2.2em, itemsep=1pt]
  ...
  \item a \textbf{construction portfolio} (\S\ref{sec:igvnd_construction})
        that seeds each restart from a different priority rule;
...
Algorithm~\ref{alg:igvnd} states the method at a high level.  Each restart
draws a seed from the portfolio, optimises it first in the zero-movement
regime to obtain a guaranteed-feasible \emph{floor}, and then polishes it
in the manoeuvre-aware regime, retaining the polished schedule only if an
...
best certified schedule over all restarts is returned.
...
preprocess $I$; build the construction portfolio
   $\mathcal{C} = (\rho_1, \ldots, \rho_6)$ (\S\ref{sec:igvnd_construction})\;
set the number of restarts $n_s = 8$ if $R \le 10$, $\;4$ if $R \le 20$,
   $\;3$ otherwise\;
$z^\star \leftarrow +\infty$\;
\For{$i = 1$ \KwTo $n_s$ \textbf{\emph{while}} time remains}{
  allot the time share $T/n_s$ to restart $i$\;
  $(\pi, \sigma) \leftarrow \textsc{Construct}(\rho_{1 + (i-1 \bmod 6)})$
...
% ------------------------------------------------------------------------------
\subsection{Construction portfolio}
\label{sec:igvnd_construction}
% ------------------------------------------------------------------------------

Each restart builds its seed from a different construction rule, so the
restarts differ by construction and not only by the perturbation source.
Five rules use greedy insertion under a fixed priority order $\rho$ ---
$-T_r$ (the NEH makespan rule), $L_r$ (earliest due date), the slack
$L_r - E_r - T_r$, the critical ratio $L_r / T_r$, and a rank blend of
these --- and a sixth uses regret-2 insertion, a dynamic order that
repeatedly commits the aircraft whose second-best position is much worse
than its best (Algorithm~\ref{alg:construct}).  The due-date and regret-2
rules steer tight-target aircraft into early slots, which is the lever the
delay-priority profile needs; the NEH rule remains the strong makespan
seed.  In both variants the cost of a tentative insertion is the value
...
\SetKwInput{KwData}{Input}
\SetKwInput{KwResult}{Output}
\KwData{a priority order $\rho$ (greedy variant), or none (regret-2 variant).}
...
\Indm
\BlankLine
\textbf{Regret-2 insertion:}\;
\Indp
  $\pi \leftarrow \emptyset$;\quad $\sigma \leftarrow ()$\;
  \While{some aircraft is unplaced}{
    \ForEach{unplaced $r$}{
      let $z^{(1)}_r \le z^{(2)}_r$ be the two lowest values of
         $z_0\big(\pi \cup \{r \mapsto p\}\big)$ over $p \in \calP$\;
    }
    $r^\star \leftarrow \argmax_r \big(z^{(2)}_r - z^{(1)}_r\big)$
       \tcp*{largest regret}
    place $r^\star$ at its best position; append $r^\star$ to $\sigma$\;
  }
  \Return $(\pi,\ \sigma)$\;
\Indm
\caption{\textsc{Construct} --- portfolio seed generators.}
\label{alg:construct}
\end{algorithm}
