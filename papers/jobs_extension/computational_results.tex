...
Table~\ref{tab:gap_profile} in both directions: $-33\%$ on \emph{Triangle}
and $+11\%$ on \emph{Two rows}.  Neither reflects a large physical
difference.  On these configurations the optimal delay is zero or near
zero, so $z_{\mathrm{MILP}}$ reduces to the lightly weighted makespan and
movement terms, and a difference of a few delay units --- weighted $100$
--- swamps that baseline: the $-33\%$ mean is driven by a single seed at
$-277\%$, on which IGVND concedes exactly two delay units, while IGVND
ends strictly better on five of the ten seeds.  The per-component view
confirms the scale: the mean $\Delta D$ in Table~\ref{tab:components} is
$+0.15$ delay units on \emph{Triangle} and $-0.2$ on \emph{Two rows}.
The positive side of these rows has a second ingredient: both
configurations are solved to proven optimality by the MILP
(Table~\ref{tab:milp_conv}), yet IGVND ends strictly below the reported
optimum on several seeds.  This is the $\eta$-clearance conservatism
discussed in \S\ref{sec:milp}: the model requires accesses to clear a
front stay by a full $\eta$, while the problem semantics --- and the
compliance checker --- accept accesses in the band immediately outside
the stay, which the heuristic exploits.  A per-seed audit of the largest
case (\emph{Two rows}, seed 8, a $47\%$ per-seed difference) confirms
that the heuristic schedule is model-infeasible only through four such
band accesses, each placed $\varepsilon$ away from a front stay.
...
of them ending strictly below the MILP's reported objective, again through
the $\eta$-clearance conservatism of \S\ref{sec:milp}: the certified
optimum is the optimum of the model's slightly restricted feasible set.  Two ingredients closed these gaps: a
