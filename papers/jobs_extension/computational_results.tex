...
simply the one that packs the schedule tightest.  Here IGVND matches the MILP
exactly on the small instances the exact solver can close ($R{=}5$ and the
loose $R{=}10$ cases, $|g| \le 0.2\%$), wins outright on \emph{Chain}
($-10.4\%$) and \emph{Hub} ($-4.0\%$) at $R{=}10$, and dominates at $R{=}20$
and $R{=}30$.  The only entries where IGVND trails under movement-priority are
sub-$1\%$ losses on the tightest small instances, at the level of the search's
run-to-run noise and inflated by the tiny optimal objective; the
concentric-nesting builder of \S\ref{sec:igvnd_safety} is what keeps the
zero-movement schedule tightly packed on the blocking topologies.
