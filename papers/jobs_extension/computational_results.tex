...
up: it exceeds $60\%$ at $R{=}20$, surpasses $75\%$ at $R{=}30$, and reaches
...
converged $R{=}10$ cases.  The makespan-priority column, where the MILP
...
loosely-constrained triangle under delay-priority ($-34\%$ mean, driven by
two seeds at $\approx -187\%$).  Here the MILP drives the total delay to
...
gap: on the two extreme seeds IGVND concedes only $1.5$ delay units, and it
...
fleet size: the relative gap rises from roughly $+5\%$ to $+18\%$ at
$R{=}20$ to $+14\%$ to $+44\%$ at $R{=}30$, across both the \emph{Two rows}
...
and dominates at $R{=}20$ and $R{=}30$ ($+7\%$ to $+35\%$, with large negative
$\Delta m$ and $\Delta D$ --- hundreds of objective units in absolute terms).
Two kinds of residual remain at $R{=}10$.  It \emph{trails} on the tight
\emph{Chain} ($-8.3\%$, $\Delta m \approx +10$) and \emph{Hub} ($-3.7\%$,
$\Delta m \approx +2$) topologies, where the MILP incumbent packs the
zero-movement schedule slightly tighter than the earliest-feasible decode
achieves; since the MILP is itself unconverged there ($82\%$ and $32\%$
optimality gaps), this is a comparison of two feasible solutions, not a
defeat against a proven optimum, though it is a genuine residual weakness of
the zero-movement packing on linear and star blocking.  On the loose
\emph{Triangle} at $R{=}10$, by contrast, the MILP \emph{does} prove
optimality, and IGVND concedes on three of ten seeds ($-4.1\%$ mean, worst
seed $-26\%$; $5$--$16$ objective units): there the optimum stretches
aircraft stays with inter-job idle to enable a tighter nesting, a schedule
shape the tight-packing decoder cannot represent.  This is the one
configuration family where the heuristic measurably loses to a certified
optimum, and it delimits the method's remaining structural gap.
