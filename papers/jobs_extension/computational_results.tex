...
fleet size: the relative gap rises from roughly $+5\%$ to $+20\%$ at
$R{=}20$ to $+19\%$ to $+44\%$ at $R{=}30$, across both the \emph{Two rows}
...
on the instances the exact solver can close ($R{=}5$ and the loose/converged
$R{=}10$ cases, $|g| \lesssim 1\%$) and dominates at $R{=}20$ and $R{=}30$
($+7\%$ to $+35\%$, with large negative $\Delta m$ and $\Delta D$ ---
hundreds of objective units in absolute terms).  It does, however,
\emph{trail} on the tight \emph{Chain} ($-10.4\%$, $\Delta m \approx +13$) and
\emph{Hub} ($-4.0\%$, $\Delta m \approx +4$) topologies at $R{=}10$, where the
MILP incumbent packs the zero-movement schedule slightly tighter than the
earliest-feasible decode achieves; since the MILP is itself unconverged there
($82\%$ and $32\%$ optimality gaps), this is a comparison of two feasible
solutions, not a defeat against a proven optimum, but it is a genuine residual
weakness of the zero-movement packing on linear and star blocking.  The
sub-$4\%$ losses at $R{=}5$ are small-denominator noise (a one-unit difference
on an objective of $\approx 36$).
