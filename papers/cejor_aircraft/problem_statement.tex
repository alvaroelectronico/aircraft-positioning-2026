...
uninterrupted block.  In practice this work is an ordered sequence of jobs
whose per-job timings can be reconstructed deterministically once the
aircraft start time is known (\S\ref{sec:job_recovery}); because the
operationally relevant quantities---makespan, delay, and blocking
interactions---depend only on the aggregated aircraft block, the problem
can be stated entirely at the aircraft level.
...
The objective is a weighted combination of three criteria: the
\emph{makespan} $m = \max_{r \in \calR} f_r$, the total \emph{delay}
$\sum_{r \in \calR} v^D_r$ with per-aircraft delay
$v^D_r = \max(0,\, f_r - L_r)$, and the number $n$ of blocking movements
induced above.  These are scalarised with user-defined weights
$W^M, W^D, W^S \ge 0$ and the resulting cost minimised; the formal
objective is stated with the model as~\eqref{eq:milpobj}.
