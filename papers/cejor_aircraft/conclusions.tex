...
default movement-priority profile returns essentially zero blocking
...
Two caveats bound these conclusions.  The benchmark is synthetic and
deliberately congested, and fixes the number of positions at $P=5$ with
one geometry per topology, so the absolute magnitudes are indicative
rather than operational forecasts; and the weight profiles, though
motivated by maintenance practice, are exogenous inputs rather than
learned from data.  Within these bounds, the experiments return to the
motivating gap: where planners today rely on positioning rules of thumb,
a fast solver portfolio delivers high-quality, movement-aware schedules
in seconds.
