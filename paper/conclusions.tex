...
above 90\,\%, including the unconstrained \emph{None} topology, which
points to bound weakness rather than blocking interactions as the
dominant source of difficulty.  At $R{=}20$ the solver exhausts the
budget with a gap close to $100\,\%$, and at $R{=}30$ it aborts before
returning any feasible incumbent, so the heuristic pipeline becomes the
only available source of solutions for the largest configurations.  On
the $R{=}10$ configurations with non-trivial blocking the heuristic
pipeline cuts the best feasible MILP objective by 5--25\,\%, and at
$R{=}20$ the gap widens by an order of magnitude (e.g.\ $\bar f = 635$
versus $2425$ on \emph{Triangle}, and $881$ versus $4676$ on
...
\emph{Triangle $R{=}30$}, for instance, FAS lags Topo by $2.2\,\%$ and
