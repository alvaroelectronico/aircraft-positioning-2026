...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 5: block swap.  For each pair of positions (p, q),
        # swap the ENTIRE aircraft sets between them.  This is a
        # multi-aircraft move (often 4-6 aircraft simultaneously) that
        # LS cannot compose from single-move or 2-opt swap on basins
        # like chain_R10 where (3,1,1,2,3) needs to become (2,2,2,2,2)
        # via several coordinated moves.
        for i in range(len(positions)):
            if not _time_left():
                break
            p = positions[i]
            aids_at_p = [aid for aid, x in assignment.items() if x == p]
            for j in range(i + 1, len(positions)):
                q = positions[j]
                aids_at_q = [aid for aid, x in assignment.items() if x == q]
                if not aids_at_p and not aids_at_q:
                    continue
                if not aids_at_p or not aids_at_q:
                    # Swap is just a single-side move when one side empty;
                    # skip — op 1 already handles single moves.
                    continue
                trial = dict(assignment)
                for aid in aids_at_p:
                    trial[aid] = q
                for aid in aids_at_q:
                    trial[aid] = p
