...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 4: idle-gap insertion.  Try delaying each aircraft's
        # earliest start by Δ ∈ {2, 5, 10, 20, 50}.  Holding a front
        # aircraft back can save more downstream rear delay than it
        # costs in own delay — this is Mode-B in spirit (the heuristic
        # itself doesn't have to model the inter-job gap explicitly;
        # the rebuild just sees a later start and replans).  Also try
        # CLEARING an override (Δ = 0) so the search can back out of
        # a previously productive delay if it stops paying.
        deltas = (0.0, 2.0, 5.0, 10.0, 20.0, 50.0)
        for aid in aircraft_ids:
            if not _time_left():
                break
