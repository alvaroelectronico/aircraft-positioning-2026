...
def _solve_single(
    ...
    best_sol, best_obj, best_overrides = _local_search(
...
def _local_search(
    ...
    # Carry forward overrides from a previous LS call when LNS provides
    # them; this lets idle-gap moves accumulate across LNS perturbations
    # instead of being rediscovered from scratch each time.  Only keep
    # entries whose aircraft are still in the current assignment.
    start_overrides: dict[str, float] = {}
    if seed_overrides:
        for aid, d in seed_overrides.items():
            if aid in assignment:
                start_overrides[aid] = d
    ...
    return best_sol, best_obj, start_overrides
