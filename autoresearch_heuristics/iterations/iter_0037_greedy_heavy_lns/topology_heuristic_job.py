...
def _solve_single(
    ...
    while _remaining() > 0.0:
        ...
        # Six-mode cycling, biased toward greedy repair (50% of cycles),
        # which is structurally productive on dense topologies like
        # full_R10 where uniform-random repair lands in poor basins.
        #   0, 2, 4: random destroy + greedy repair  (50 %% of LNS iters)
        #   1, 3:    random destroy + uniform repair (33 %% — balance discovery)
        #   5:       FULL restart (destroy all, uniform repair) — basin escape
        mode = kick_idx % 6
        kick_idx += 1
