...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 5: pair idle-gap.  For each pair (a, b) of aircraft,
        # try delaying both to start at base + Δ for the same Δ.  This
        # captures synchronised "slot 2 starts after slot 1 clears"
        # moves that single-aircraft idle-gap can't reach when each
        # individual delay (a alone, or b alone) makes things slightly
        # worse but the simultaneous move improves the objective.
        pair_deltas = (5.0, 10.0, 20.0, 50.0, 100.0)
        for i in range(len(aircraft_ids)):
            if not _time_left():
                break
            ai = aircraft_ids[i]
