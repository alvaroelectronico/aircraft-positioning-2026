...
def _solve_single(
    ...
    # strict improvement.  Four (destroy, repair) combos in round-robin:
    #   0: random destroy + greedy repair    — original iter_0003 behaviour.
    #   1: random destroy + uniform repair   — iter_0011: balance-friendly.
    #   2: worst-removal destroy + greedy    — Shaw/Ropke intensification.
    #   3: worst-removal destroy + uniform   — combines targeted + balanced.
    ...
    while _remaining() > 0.0:
        ...
        if mode < 2:
            ...
            destroyed = _destroy_worst(best_sol, best_assignment, k, instance, rng)
...
def _destroy_worst(
    ...
    """Pick the k aircraft with the largest delay in ``sol``; expand the set
    with each picked aircraft's blocking-arc neighbours (positions reachable
    from / to the picked aircraft's position), capped at 2k total.  Falls
    back to a uniform sample when delays are all zero."""
    ...
    for aid in picks:
        ...
        if p is None:
            continue
        for q in list(arc_from.get(p, [])) + list(arc_to.get(p, [])):
    ...
    if not chosen:
        ...
        return set(rng.sample(aids, min(k, len(aids))))
    return chosen
