...
def _solve_single(
    ...
    """One GRASP iteration: construct, run the LS portfolio, return the best."""
    ...
    # Local-search portfolio with restart-on-improvement.
    elapsed = time.perf_counter() - t0
    remaining = max(0.0, budget_s - elapsed)
    final_sol, final_obj = _local_search(
        assignment, None, instance, params, remaining,
    )
...
def _local_search(
    ...
    Three operators applied in order of cost (cheapest first):
        ...
        3. **intra-pos adj-swap** — swap two adjacent same-position aircraft
                                 in the scheduling sequence.
        4. **intra-pos insertion** — pop one aircraft out of its same-position
                                 block and reinsert at every other slot.
        5. **intra-pos non-adj swap** — swap any two same-position aircraft
                                 in the sequence, including non-adjacent
                                 pairs (covers what op 3 misses for blocks
                                 of three or more).
        6. **EDD repair**     — replace each position's block with one of
                                 three full reorderings: EDD (target_finish),
                                 slack-ASC, delay-ratio-DESC.

    On any improvement, the loop restarts from operator 1.  The search
    terminates when a full pass through all six operators finds no
    ...
    while improved and _time_left():
        ...
        # Pre-compute per-position blocks of aircraft (in scheduling order).
        blocks = _position_blocks(order, assignment)

        # Operator 4: intra-position insertion.
        # Pop each same-position aircraft and reinsert at every other slot.
        for pos, pos_aids in blocks.items():
            ...
            for src in range(len(pos_aids)):
                for dst in range(len(pos_aids)):
                    ...
                    trial_order = _splice_block(order, pos_set, reordered)
                    sol = _rebuild_job(assignment, instance, params, order=trial_order)
                    obj = _objective_job(sol, params)
                    if _accept(sol, obj):
                        order    = trial_order
        ...
        # Operator 5: non-adjacent intra-position swap.
        for pos, pos_aids in blocks.items():
            if not _time_left() or len(pos_aids) < 3:
                continue  # adjacent case is covered by op 3
            pos_set = set(pos_aids)
            for i in range(len(pos_aids)):
                for j in range(i + 2, len(pos_aids)):
                    swapped = list(pos_aids)
                    swapped[i], swapped[j] = swapped[j], swapped[i]
                    trial_order = _splice_block(order, pos_set, swapped)
                    sol = _rebuild_job(assignment, instance, params, order=trial_order)
                    obj = _objective_job(sol, params)
                    if _accept(sol, obj):
                        order    = trial_order
        ...
        # Operator 6: EDD repair per position.
        # Three candidate full orderings per block: EDD (target_finish ASC),
        # slack ASC, delay_ratio DESC.  Per-aircraft delay is read from the
...
def _position_blocks(
    ...
    """Return {position: [aircraft ids in scheduling order]} for that position."""
    blocks: dict[str, list[str]] = {}
    for aid in order:
        p = assignment.get(aid)
        if p is None:
            continue
        blocks.setdefault(p, []).append(aid)
    return blocks
...
def _splice_block(
    ...
    """Return a copy of ``order`` with the aircraft in ``block_ids`` replaced,
    in their original slots, by the sequence ``new_block``."""
    it = iter(new_block)
    return [next(it) if aid in block_ids else aid for aid in order]
