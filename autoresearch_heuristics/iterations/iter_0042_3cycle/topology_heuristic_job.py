...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 5: 3-cycle position rotation.  For each ordered triple
        # of aircraft with distinct positions, rotate forward.  Reaches
        # 3-aircraft basin escapes that single-move and 2-opt swap can't.
        n = len(aircraft_ids)
        for i in range(n):
            if not _time_left():
                break
            ai = aircraft_ids[i]; pi = assignment[ai]
            for j in range(n):
                if j == i:
                    continue
                aj = aircraft_ids[j]; pj = assignment[aj]
                if pj == pi:
                    continue
                for kk in range(n):
                    if kk == i or kk == j:
                        continue
                    ak = aircraft_ids[kk]; pk = assignment[ak]
                    if pk == pi or pk == pj:
                        continue
                    trial = dict(assignment)
                    trial[ai], trial[aj], trial[ak] = pj, pk, pi
