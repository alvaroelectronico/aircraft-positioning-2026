...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 5: cyclic position rotation.  Try the n_pos cyclic
        # rotations of the position assignment (every aircraft at P_i
        # moves to P_{(i+s) mod n_pos}).  Different distribution shape
        # against the topology graph; LS can'"'"'t reach via single moves.
        n_pos = len(positions)
        pos_idx = {p: i for i, p in enumerate(positions)}
        for s in range(1, n_pos):
            if not _time_left():
                break
            trial = {aid: positions[(pos_idx[p] + s) % n_pos]
