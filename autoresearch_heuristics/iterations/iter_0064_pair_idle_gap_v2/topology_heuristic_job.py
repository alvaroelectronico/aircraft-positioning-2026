...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 5: pair idle-gap.  Synchronous delay for two aircraft
        # at the same time — captures slot-2 staggering that single
        # idle-gap can'"'"'t reach when each individual delay regresses.
        pair_deltas = (5.0, 10.0, 20.0, 50.0)
        for i in range(len(aircraft_ids)):
            if not _time_left():
                break
            ai = aircraft_ids[i]
