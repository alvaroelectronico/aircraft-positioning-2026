...
def _solve_single(
    ...
    while _remaining() > 0.0:
        ...
        # Eight-mode cycling — keep iter_0029's mix but extend with two more
        # cycles of random+greedy so the construction-respecting moves get
        # more share again (helps full_R10).
        #   0, 3, 6: random destroy + greedy repair
        #   1, 4, 7: random destroy + uniform repair
        #   2:       topdest destroy + uniform repair
        #   5:       full restart + uniform repair
        mode_8 = kick_idx % 8
        mode = {0: 0, 1: 1, 2: 2, 3: 0, 4: 1, 5: 3, 6: 0, 7: 1}[mode_8]
        kick_idx += 1
        if mode == 0:
