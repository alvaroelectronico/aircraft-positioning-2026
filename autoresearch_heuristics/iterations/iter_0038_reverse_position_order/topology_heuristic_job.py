...
def _local_search(
    ...
    while improved and _time_left():
        ...
        # Operator 5: reverse position order.  For each position with ≥3
        # aircraft, try reversing their order in the schedule.  This is a
        # global reordering within a position — distinct from adj-swap
        # (which only swaps two) and from idle-gap (which adjusts timing).
        blocks_rev: dict[str, list[str]] = {}
        for aid in order:
            blocks_rev.setdefault(assignment.get(aid, "?"), []).append(aid)
        for pos, pos_aids in blocks_rev.items():
            if not _time_left() or len(pos_aids) < 3:
                continue
            pos_set = set(pos_aids)
            reordered = list(reversed(pos_aids))
            it = iter(reordered)
