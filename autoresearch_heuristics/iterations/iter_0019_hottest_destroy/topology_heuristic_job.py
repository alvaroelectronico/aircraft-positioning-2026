...
def _solve_single(
    ...
    while _remaining() > 0.0:
        ...
        #   2: topdest destroy + uniform repair  (iter_0018: most-populated
        #      positions, forces rebalance)
        #   3: hottest destroy + uniform repair  (new: position whose aircraft
        #      accumulate the most delay; forces re-thinking the worst spot)
        mode = kick_idx % 4
        kick_idx += 1
        if mode == 0:
            ...
            delay_at_pos: dict[str, float] = {}
            aids_at_pos: dict[str, list[str]] = {}
            for a in best_sol["aircraft"]:
                p = a["position"]
                delay_at_pos[p] = delay_at_pos.get(p, 0.0) + a.get("delay", 0.0)
                aids_at_pos.setdefault(p, []).append(a["id"])
            ranked_d = sorted(delay_at_pos.items(), key=lambda kv: -kv[1])
            destroyed = [aid for p, _ in ranked_d[:2] for aid in aids_at_pos.get(p, [])]
            repair = "uniform"
