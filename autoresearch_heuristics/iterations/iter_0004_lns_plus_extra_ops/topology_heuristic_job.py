...
def _position_blocks(
    ...
    """{position: [aircraft ids in scheduling order]} for that position."""
...
def _splice_block(
    ...
    """Copy of ``order`` with the aircraft in ``block_ids`` replaced, in
    their original slots, by the sequence ``new_block``."""
    it = iter(new_block)
